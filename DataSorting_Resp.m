function [kdata_Under3,Traj_Under3,DensityComp_Under3,Res_Signal_Under]=DataSorting_Resp(kdata,Traj,DensityComp,Res_Signal,nline,para, nResp, nCard);
...
if (tres>1)
    for ii=1:nResp
        ...
        tmp1=reshape(tmp1,[nx,nline*tres,nc,para.ntres]);
        tmp2=reshape(tmp2,[nx,nline*tres,para.ntres]);
        tmp3=reshape(tmp3,[nx,nline*tres,para.ntres]);
...
tcar=floor(para.ntres/nCard);
if (tcar>1)
    for ii=1:nCard
        tmp1=kdata_Under2(:,:,:,(ii-1)*tcar+1:ii*tcar,:);
        tmp2=Traj_Under2(:,:,(ii-1)*tcar+1:ii*tcar,:);
        tmp3=DensityComp_Under2(:,:,(ii-1)*tcar+1:ii*tcar,:);
        
        tmp1=permute(tmp1,[1,2,4,3,5]);
        
        tmp1=reshape(tmp1,[nx,nline*tcar*tres,nc,nResp]);
        tmp2=reshape(tmp2,[nx,nline*tcar*tres,nResp]);
        tmp3=reshape(tmp3,[nx,nline*tcar*tres,nResp]);
