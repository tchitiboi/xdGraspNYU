function [kdata_Under2,Traj_Under2,DensityComp_Under2,p2]=DataSorting_Resp_Card_RR(kdata,Traj,DensityComp,Res_Signal,Res_Signal_P, cycleLabels, labels, nline,para, nResp, respBin, nCard);
...
kdata_Under2 = cell(nCard,labels);
Traj_Under2 = cell(nCard,labels);
DensityComp_Under2 = cell(nCard,labels);
p2 = cell(nCard,labels);
minSpokes = 35;
  ...
  for label=1:labels
    %for resp = 1:nResp
        for card = 1:nCard
            ...
            ind_b=[];
            for j = 1:para.ntres
               ind_b = [ind_b,find(Res_Signal_percycle{j,card,label} == respBin)];
               ind_a = [ind_a,repmat(j,1,length(find(Res_Signal_percycle{j,card,label} == respBin)))];
            end
            ind_a_extra = [];
            ind_b_extra = [];
            ...
            if (length(ind_a)<minSpokes)   
                ...
                ind_b3=[];
                for j = 1:para.ntres
                  ind_b3 = [ind_b3,find(Res_Signal_percycle{j,card,label} ~= respBin)];
                  ind_a3 = [ind_a3,repmat(j,1,length(find(Res_Signal_percycle{j,card,label} ~= respBin)))];
                end
                p = zeros(length(ind_a3),2);
                for i = 1:length(ind_a3)                    
                    p(i,1) = Res_Signal_P_percycle{ind_a3(i),card,label}(ind_b3(i),respBin);
                    p(i,2) = i;
                end
                p = sortrows(p,-1);
            ...
            for i = 1:length(ind_a)
                ...
                tmp4(i)=squeeze(Res_Signal_P_percycle{ind_a(i),card,label}(ind_b(i),respBin));
            end
            ...
            kdata_Under2{card,label} = tmp1;
            Traj_Under2{card,label} = tmp2;
            DensityComp_Under2{card,label} = tmp3;
            p2{card,label} = tmp4;
            size(tmp1)
