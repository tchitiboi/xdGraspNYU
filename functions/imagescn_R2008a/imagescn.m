...
% 	end
	if isempty(scale)
		if Nd>4
            imagesc(I(:,:,i0,j0)); axis square; axis off;axis image
		else 		
            imagesc(I(:,:,k)); axis square; axis off;axis image
		end
	elseif size(scale,1)==1
		if Nd>4
            imagesc(I(:,:,i0,j0),scale); axis square; axis off;axis image
        else	
            imagesc(I(:,:,k),scale); axis square;axis off,
            imagesc(I(:,:,k),scale); axis image;axis off,%zoom(1.2)
		end
	elseif size(scale,1)==min(N,N1*N2);
		if Nd>4
