function imagescnzoom(I, scale, dims, FigureWidth, timedimension)
...
% 	end
	if isempty(scale)
		if Nd>4
            imagesc(I(:,:,i0,j0)); axis image; axis off;axis square
		else 		
            imagesc(I(:,:,k)); axis image; axis off;axis square
		end
	elseif size(scale,1)==1
		if Nd>4
            imagesc(I(:,:,i0,j0),scale); axis image; axis off;axis square
        else	
%             imagesc(I(:,:,k),scale); axis image;axis off,axis square
            imagesc(I(:,:,k),scale); axis image;axis off,axis square,zoom(2.5)
		end
	elseif size(scale,1)==min(N,N1*N2);
		if Nd>4
            imagesc(I(:,:,i0,j0),scale(k,:)); axis image; axis off;axis square
		else		
            imagesc(I(:,:,k),scale(k,:)); axis image;axis off,axis square
		end
	end
    if moviemodeflag==1        
        if Nd==3
